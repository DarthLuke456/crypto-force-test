-- =====================================================
-- CORRECCIONES DE SEGURIDAD PARA SUPABASE - VERSIÓN FINAL
-- =====================================================
...
-- =====================================================
-- 1. HABILITAR RLS EN TABLA REFERRAL_HISTORY
-- =====================================================
-- Error: RLS Disabled in Public
-- Tabla: public.referral_history
-- Estructura: referrer_code, referred_code (no referrer_id, referred_id)
...
CREATE POLICY "Users can view their own referral history" ON public.referral_history
    FOR SELECT USING (
        auth.uid() = (SELECT id FROM public.users WHERE referral_code = referrer_code) OR 
        auth.uid() = (SELECT id FROM public.users WHERE referral_code = referred_code)
    );
...
-- =====================================================
-- 2. CONFIGURAR SEARCH_PATH INMUTABLE EN FUNCIONES
-- =====================================================
...
CREATE OR REPLACE FUNCTION public.process_new_referral(referrer_code TEXT, referred_code TEXT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, auth
...
DECLARE
    ...
    referred_id UUID;
BEGIN
    -- Obtener IDs de los códigos de referencia
    SELECT id INTO referrer_id FROM public.users WHERE referral_code = referrer_code;
    SELECT id INTO referred_id FROM public.users WHERE referral_code = referred_code;
    
    -- Procesar nueva referencia
    INSERT INTO public.referral_history (referrer_code, referred_code, created_at)
    VALUES (referrer_code, referred_code, NOW());
...
-- =====================================================
-- 3. MOVER EXTENSIÓN CITEXT DEL SCHEMA PUBLIC
-- =====================================================
...
-- =====================================================
-- 4. CONFIGURAR OTP EXPIRY (MENOS DE 1 HORA)
-- =====================================================
...
-- =====================================================
-- 5. HABILITAR PROTECCIÓN CONTRA CONTRASEÑAS COMPROMETIDAS
-- =====================================================
