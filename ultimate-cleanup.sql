-- =====================================================
-- ULTIMATE CLEANUP: ELIMINAR TODAS LAS FUNCIONES Y RECREARLAS
-- =====================================================
-- Este script elimina TODAS las funciones del schema public y las recrea desde cero

-- =====================================================
-- 1. ELIMINAR TODAS LAS FUNCIONES DEL SCHEMA PUBLIC
-- =====================================================

-- Obtener todas las funciones y eliminarlas una por una
DO $$
DECLARE
    func_record RECORD;
    func_signature TEXT;
BEGIN
    -- Obtener todas las funciones del schema public
    FOR func_record IN 
        SELECT 
            routine_name,
            routine_type,
            string_agg(
                CASE 
                    WHEN parameter_name IS NULL THEN ''
                    ELSE parameter_name || ' ' || COALESCE(p.data_type, 'unknown')
                END, 
    ...
    LOOP
        ...
        IF func_record.parameters IS NULL OR func_record.parameters = '' THEN
            func_signature := 'public.' || func_record.routine_name || '()';
        ELSE
            func_signature := 'public.' || func_record.routine_name || '(' || func_record.parameters || ')';
        END IF;
        ...
        EXCEPTION
            WHEN OTHERS THEN
                ...
                EXCEPTION
                    WHEN OTHERS THEN
                        -- Si aún falla, continuar con la siguiente
                        CONTINUE;
                END;
        END;
    END LOOP;
...
SELECT NOW() as current_time, 'All functions eliminated' as status;
