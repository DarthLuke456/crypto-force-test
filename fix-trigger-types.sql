-- Script para arreglar el error de tipos en el trigger
...
-- 3. Crear función simple para insertar usuario (con tipos corregidos)
...
BEGIN
  ...
  user_nickname := COALESCE(NEW.raw_user_meta_data->>'nickname', 'usuario' || SUBSTRING(NEW.id::TEXT FROM 1 FOR 8));
  
  -- Generar código de referido
  user_referral_code := generate_referral_code(user_nickname);
  
  -- Insertar en public.users (con tipos corregidos)
