-- =====================================================
-- CORRECCIONES DE SEGURIDAD PARA SUPABASE - VERSIÓN CON SINTAXIS CORRECTA
-- =====================================================
...
BEGIN
    ...
    SET referral_code = referral_code_to_assign,
        total_referrals = 0,
        user_level = 'acolito' -- Nivel por defecto
    WHERE id = NEW.id;
...
BEGIN
    UPDATE public.users
    SET user_level = 'founder'
    WHERE email LIKE '%@cryptoforce.com%' AND user_level != 'founder';
END;
$$;
...
AS $$
BEGIN
    IF OLD.user_level = 'maestro' AND NEW.user_level != 'maestro' THEN
...
AS $$
BEGIN
    IF OLD.user_level IN ('founder', 'maestro') AND NEW.user_level NOT IN ('founder', 'maestro') THEN
...
AS $$
BEGIN
    IF NEW.total_referrals >= 10 AND NEW.user_level = 'acolito' THEN -- Ejemplo: 10 referidos para ser maestro
        NEW.user_level = 'maestro';
    END IF;
    RETURN NEW;
END;
$$;
...
BEFORE UPDATE OF user_level ON public.users
FOR EACH ROW
